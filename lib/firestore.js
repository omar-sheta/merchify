 ...
 *     - createdAt: timestamp
 *     - totalPrice: number
 *     - likeCount: number (aggregate count of likes)
 * 
 * users/ (collection)
...
export async function likeDesign(userId, designId) {
  try {
    ...
    // Add design to user's likes array
    await updateDoc(userRef, {
      likes: arrayUnion(designId),
      updatedAt: serverTimestamp(),
    })
    
    // Increment the design's like count
    const designSnap = await getDoc(designRef)
    if (designSnap.exists()) {
      const currentLikes = designSnap.data().likeCount || 0
...
export async function unlikeDesign(userId, designId) {
  try {
    ...
    // Remove design from user's likes array
    await updateDoc(userRef, {
      likes: arrayRemove(designId),
      updatedAt: serverTimestamp(),
    })
    
    // Decrement the design's like count (don't go below 0)
    const designSnap = await getDoc(designRef)
    if (designSnap.exists()) {
      const currentLikes = designSnap.data().likeCount || 0
      await updateDoc(designRef, {
