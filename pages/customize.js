      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <Card padding="p-6">
            ...
            <div className="relative bg-gradient-to-br from-gray-50 to-gray-200 rounded-xl p-6 mb-6 min-h-[380px] flex items-center justify-center border border-gray-200">
              {generatedMockup ? (
                <div className="text-center w-full">
                  ...
                  />
                  <p className="text-sm text-green-700 font-medium mt-4">✨ Generated by AI Designer</p>
                </div>
              ) : (
                <div className="text-center">
            ...
            <Button onClick={generateMockup} disabled={isGenerating} size="lg" className="w-full mb-4">
              {isGenerating ? (
                <>
          ...
          <div className="space-y-6">
            {/* Product Selection */}
            <Card padding="p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wide">Product</h3>
              <div className="grid grid-cols-2 gap-4">
                {products.map((product) => (
                  <button
                    key={product.id}
                    onClick={() => setSelectedProduct(product.id)}
                    className={`group p-4 rounded-xl border-2 transition-all text-left ${selectedProduct === product.id ? 'border-purple-600 bg-purple-50 shadow-sm' : 'border-gray-200 hover:border-gray-300'}`}
                  >
              ...
              <div className="flex gap-4">
                {colors.map((color) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-full border-4 transition-all ${selectedColor === color.id ? 'border-purple-600 scale-110' : 'border-gray-300'}`}
            ...
            {/* Size & Quantity */}
            <Card padding="p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wide">Size & Quantity</h3>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">Prompt for AI Designer</h3>
              <p className="text-sm text-gray-600 mb-4">Add instructions or creative direction (style, color, small effects). Kept short for best results.</p>
              <textarea
                value={prompt}
            ...
            {/* Price Summary */}
            <div className="bg-gradient-to-br from-purple-600 to-violet-700 rounded-2xl shadow-2xl p-8 text-white">
              <div className="flex justify-between items-center mb-6">
                <span className="text-lg">Total Price:</span>
                <span className="text-4xl font-bold">${totalPrice}</span>
              </div>

              <Button onClick={proceedToCheckout} variant="secondary" size="lg" className="w-full mb-3">
                Proceed to Checkout
